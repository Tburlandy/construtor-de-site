...
export const ProofBar = ({ onOpenPopup }: ProofBarProps) => {
  ...
  const benefits = content?.proofBar?.cards?.length
    ? content.proofBar.cards
    : [
        {
  ...
  const benefitIcons = [Star, Shield, Clock, Award, CheckCircle2, Users];
      ...
      <div className="max-w-6xl mx-auto">
        <RevealOnScroll>
          <h2 className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold mb-8 md:mb-12 text-left">
            {content?.proofBar?.titlePrefix || 'Escolha a empresa'}{' '}
            <span className="text-primary">{content?.proofBar?.titleHighlight || 'mais bem avaliada'}</span>{' '}
            {content?.proofBar?.titleSuffix || 'do Rio de Janeiro e não tenha dor de cabeça'}
          </h2>
        </RevealOnScroll>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                {benefits.map(({ title, description }, idx) => {
                  const Icon = benefitIcons[idx] || Star;
                  return (
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4 mt-8 md:mt-10">
                ...
                >
                  {content?.proofBar?.primaryCtaLabel || 'Orçamento gratuito'}
                </Button>
                <Button 
                  onClick={() => {
                    trackCTAPopupOpen('proofbar');
                ...
                >
                  ...
                  {content?.proofBar?.secondaryCtaLabel || 'Fale no Whatsapp'}
                </Button>
              </div>
            </div>
          </RevealOnScroll>
