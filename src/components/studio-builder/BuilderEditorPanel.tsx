...
import type { Benefit, Content, FaqItem, HiddenPageSectionId, ImageLayout, Project } from '@/content/schema';
import {
  DEFAULT_FAQ_ITEMS,
...
export function BuilderEditorPanel({
  ...
  const [openFaqItemIds, setOpenFaqItemIds] = useState<string[]>([]);
  ...
  const renderSectionEditor = (sectionId: BuilderSectionId) => {
    ...
    if (sectionId === 'faq') {
      const faqItems: FaqItem[] = content.faq?.items?.length
        ? (content.faq.items as FaqItem[])
        : DEFAULT_FAQ_ITEMS;

      const patchFaqItems = (nextItems: FaqItem[]) => {
        onContentChange((current) => ({
          ...current,
          faq: {
            titlePrefix: current.faq?.titlePrefix ?? DEFAULT_FAQ_TITLE_PREFIX,
            titleHighlight: current.faq?.titleHighlight ?? DEFAULT_FAQ_TITLE_HIGHLIGHT,
            subtitle: current.faq?.subtitle ?? DEFAULT_FAQ_SUBTITLE,
            items: nextItems,
          },
        }));
      };
      ...
      return (
        <div className="grid gap-4">
          {renderInheritanceToolbar('faq')}
          <div className="grid gap-3 sm:grid-cols-2">
            <BuilderInheritedFieldChrome
              label="Título (prefixo)"
              contentPath="faq.titlePrefix"
              inheritance={inheritance}
              onResetSuccess={applyResolvedContent}
              isBusy={inheritance?.isResettingField}
            >
              <input
                className={builderInputClassName}
                value={content.faq?.titlePrefix ?? DEFAULT_FAQ_TITLE_PREFIX}
                onChange={(event) =>
                  onContentChange((current) => ({
                    ...current,
                    faq: {
                      ...
                      titleHighlight: current.faq?.titleHighlight ?? DEFAULT_FAQ_TITLE_HIGHLIGHT,
                      subtitle: current.faq?.subtitle ?? DEFAULT_FAQ_SUBTITLE,
                      items: current.faq?.items?.length ? current.faq.items : [...DEFAULT_FAQ_ITEMS],
                    },
                  }))
                }
              />
            ...
            <BuilderInheritedFieldChrome
              label="Título (destaque)"
              contentPath="faq.titleHighlight"
              inheritance={inheritance}
              onResetSuccess={applyResolvedContent}
              isBusy={inheritance?.isResettingField}
            >
              <input
                className={builderInputClassName}
                value={content.faq?.titleHighlight ?? DEFAULT_FAQ_TITLE_HIGHLIGHT}
                onChange={(event) =>
                  onContentChange((current) => ({
                    ...current,
                    faq: {
          ...
          >
            <textarea
              className={builderTextAreaClassName}
              rows={2}
              value={content.faq?.subtitle ?? DEFAULT_FAQ_SUBTITLE}
              onChange={(event) =>
                onContentChange((current) => ({
                  ...current,
                  faq: {
          ...
          >
            <Accordion
              type="multiple"
              value={openFaqItemIds}
              onValueChange={setOpenFaqItemIds}
              ...
              {faqItems.map((item, index) => (
                ...
                >
                  <AccordionTrigger className="px-3.5 py-2.5 text-left no-underline hover:no-underline">
                    <span className="line-clamp-2 text-sm font-semibold text-[var(--builder-text-primary)]">
                      {item.question.trim() || `Pergunta ${index + 1}`}
                    </span>
                  </AccordionTrigger>
                  <AccordionContent className="space-y-2 border-t border-[var(--builder-border)] px-2.5 pb-2.5 pt-2">
                    <div className="space-y-1.5">
                      <p className={builderLabelClassName}>Pergunta</p>
                      <input
                        className={builderInputClassName}
                        value={item.question}
                        onChange={(event) =>
                          patchFaqItems(
                            faqItems.map((row, rowIndex) =>
                    ...
                    <div className="space-y-1.5">
                      <p className={builderLabelClassName}>Resposta (parágrafos separados por linha em branco)</p>
                      <textarea
                        className={builderTextAreaClassName}
                        rows={6}
                        value={item.answer}
                    ...
                    <button
                      ...
                      onClick={() => patchFaqItems(faqItems.filter((_, rowIndex) => rowIndex !== index))}
                    >
                      <Trash2 className="h-4 w-4" />
                      Remover pergunta
                    </button>
          ...
          <button
            ...
            onClick={() =>
              patchFaqItems([...faqItems, { question: 'Nova pergunta', answer: 'Escreva a resposta aqui.' }])
            }
          >
            <Plus className="h-4 w-4" />
            Adicionar pergunta
